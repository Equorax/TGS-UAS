...
class snack:
    ...
    def simpan_total_snack_value(self):
         self.snack_total_value = int(self.Total_snck.get().replace(",", ""))
...
class nota:
    ...
    def nota1(self,film_terpilih,kursi_terpilih,snack_total_value):
        ...
        self.nota_bayar(film_terpilih,kursi_terpilih,snack_total_value)  # nampilin halaman penting

    def nota_bayar(self,film_terpilih,kursi_terpilih,snack_total_value):  # fungsi isinya halaman snack
        ...
        input_Harga_Total_nt.insert(0, 0) 

        # soda = Label(Frame9, text="Soda", font=("Times New Roman", 15),bg="azure4", fg="black")
        # soda.grid(row=4, column=0, padx=1, pady=5, sticky="w")

        # input_soda = Entry(Frame9, width=20, borderwidth=5, relief=GROOVE)
        # input_soda.grid(row=4, column=1, padx=10, pady=10)
        # input_soda.insert(0, 0) 

        # air = Label(Frame9, text="Air Mineral", font=("Times New Roman", 15),bg="azure4", fg="black")
        # air.grid(row=5, column=0, padx=1, pady=5, sticky="w")
        
        # input_air = Entry(Frame9, width=20, borderwidth=5, relief=GROOVE)
        # input_air.grid(row=5, column=1, padx=10, pady=10)
        # input_air.insert(0, 0) 

        cek_snack = Button(Frame9, text="Cek", command=lambda: self.harga_total_nota(input_jmlh_kur,snack_total_value,input_Harga_Total_nt))
        cek_snack.grid(row=6, column=1, padx=10, pady=10,columnspan=2)  #tombol cek film

    def harga_total_nota(self,input_jmlh_kur,snack_total_value,input_Harga_Total_nt):
        harga_kursi = int(input_jmlh_kur.get()) * 40000
        
        harga_snack_total = int(snack_total_value)

        harga_tiket = harga_kursi + harga_snack_total

        input_Harga_Total_nt.grid(row=3, column=1, padx=10, pady=10)
        input_Harga_Total_nt.insert(0, harga_tiket) 
